...
- 补偿任务生命周期编排：`/operations/console/compensation-tasks` 可聚合恢复补偿、replay job、行动项 SLA 和 OnCall 状态为统一任务板，`close_compensation_task` action 可完成人工验收关闭并落库覆盖状态
- 内存会话状态和确定性工作流状态机
