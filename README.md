...
- 取消补偿：取消交通订单、取消酒店订单、释放酒店库存
- 异常恢复：审批驳回、价格变化、库存失效、订单失败后可补偿并重新规划
...
- 推进多 Agent 协作雏形：拆出 Orchestrator、Policy、Hotel、Transport、Approval、Booking 等职责边界
- 保持现有 CLI 和单元测试兼容，让多 Agent 先作为内部编排结构演进，不破坏当前单 Agent 使用方式
- 增加跨 Agent 的任务上下文、工具调用审计和失败回传机制
- 后续再推进改签/退订、日历同步、Prometheus/OpenTelemetry 指标和评测集
